...
export default function EnrolledStudentPage() {
  ...
  const mappedStudents: EnrolledStudent[] = Array.isArray(fetchedStudents)
    ? fetchedStudents.map((student: any) => ({
        ...
        total_assigned_course: student.total_assigned_course ?? 0, // default to 0 if undefined
      }))
    : [];
            ...
            <thead className="bg-gray-50">
              <tr>
                {[
                  ...
                  { key: "total_assigned_course", label: "ASSIGNED COURSES" },
                  { key: "date_enrolled", label: "DATE ENROLLED" },
                ].map(({ key, label }) => {
                  let minWidthClass = "min-w-[205px] text-left";
                  if (key === "student_id") minWidthClass = "min-w-[230px] text-left";
                  else if (key === "total_assigned_course")
                    minWidthClass = "min-w-[210px] text-center";

                  return (
                    <th
                      key={key}
                      onClick={() => requestSort(key as keyof EnrolledStudent)}
                      className={`px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer ${minWidthClass}`}
                    >
                ...
                currentItems.map((student) => (
                  <tr key={student.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500 text-center">
                      {student.total_assigned_course}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500">
                      {formatDate(student.date_enrolled)}
                    </td>
                      <td className="px-6 py-4 text-right text-sm align-middle">
                        <button
                          onClick={() =>
                            router.push(
                              `/administrative/courses/assign-courses/${student.id}/assign`
                            )
                          }
                          className="flex items-center gap-2 px-3 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition w-37 mx-auto cursor-pointer"
                        >
                          <IoMdAdd size={16} />
                          Assign Courses
                        </button>
